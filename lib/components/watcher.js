...
Component.prototype.loadFileFunc = function (filename, isLoadAll, cb) {
    ...
    tmpParser.on('readable', function () {
        let record;
        var tmpL = self.tmpConfigDataTbl[curFileIdx];
        if (tmpL) {
            while (record = tmpParser.read()) {
                const recordStr = [...new Set(record)].join();
                if (recordStr.trim() !== '') {
                    tmpL.push(record);
                }
            }
